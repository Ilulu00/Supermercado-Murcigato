...
class CarritoCRUD:
    ...
    def agregar_producto(self, id_carrito: UUID, id_producto: UUID, cantidad: int) -> Detalle_carrito:
        ...
        """
        carrito= self.db.query(Carrito).filter(Carrito.id_carrito == id_carrito, Carrito.activo==True).first()
        if not carrito:
    ...
    def ver_carrito(self, id_carrito: UUID):
        ...
        detalle= self.db.query(Detalle_carrito).filter(Detalle_carrito.id_carrito == id_carrito).all()
    ...
    def actualizar_producto(self, id_carrito: UUID, id_producto: UUID, cantidad_nueva: int) -> Optional[Detalle_carrito]:
        ...
        detalle= self.db.query(Detalle_carrito).filter(Detalle_carrito.id_carrito == id_carrito, Detalle_carrito.id_producto == id_producto).first()
